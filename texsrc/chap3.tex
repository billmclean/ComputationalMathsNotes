...
and is therefore second-order accurate by Theorem~\ref{thm: first central diff}:
\[
...
\]
so the result follows from Theorem~\ref{thm: Taylor remainder}.
\end{proof}

\begin{figure}
\caption{Computational stencil for the Crank--Nicolson 
method~\eqref{eq: Crank-Nicolson 1d}.}\label{fig: Crank-Nicolson}
\begin{center}
\begin{tikzpicture}[scale=0.5]
\draw[->] (-1,0) -- (17,0);
\node[right] at (17,0) {$x$};
...
\]
Figure~\ref{fig: Crank-Nicolson 1d} shows the stencil for the scheme.
...
$h=\tfrac12\Delta t$, and Theorem~\ref{thm: 2nd central diff} with~$h=\Delta 
x$, we conclude that
\[
|\tau^{n-1/2}_p|
