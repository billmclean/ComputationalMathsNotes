...
A matrix~$\boldsymbol{A}=[a_{ij}]$ has \emph{upper bandwidth}~$s$ 
if $a_{ij}=0$ whenever $j-i>s$, or equivalently if $\boldsymbol{A}$ has $s$ 
non-zero superdiagonals.  Similarly, $\boldsymbol{A}$ has \emph{lower 
bandwidth}~$s$ if $a_{ij}=0$ whenever $i-j>s$, so that there are $s$ non-zero 
subdiagonals.  For example, the following matrix has upper bandwidth~$2$
and lower bandwidth~$3$:
\[
\begin{bmatrix}
 5& 2&-1& 0& 0& 0& 0\\
 1& 6& 0& 8& 0& 0& 0\\
...
with bandwith~$s$.  We will seek a \emph{band Cholesky factorization}
...
upper bandwidth~$s$.  By the definition of matrix multiplication,
\[
...
that is, if $k>\in(i,j)$, so in fact
\[
\bigl(\boldsymbol{R}^T\boldsymbol{R}\bigr)_{ij}
	=\sum_{k=1}^{\min(i,j)} r_{ki}r_{kj}.
\]
In addition, $\boldsymbol{R}$ has upper bandwidth~$s$ so $r_{ki}r_{kj}=0$ if 
$i>k+s$~or $j>k+s$.  Thus, taking account of symmetry, \eqref{eq: Chol fact}
is satisfied iff
\[
a_{ij}=\sum_{k=\max(1,j-s)}^ir_{ki}r_{kj}
	\quad\text{for $\max(1,j-s)\le i\le j\le n$.}
\]
We split the last term off the sum, writing
\[
a_{ij}=r_{ii}r_{ij}+\sum_{k=\max(1,j-s)}^{i-1}r_{ki}r_{kj}
	\quad\text{for $\max(1,j-s)\le i<j\le n$,}
\]
in the off-diagonal case, and
\[
a_{jj}=r_{jj}^2+\sum_{k=\max(1,j-s)}^{j-1}r_{kj}^2
	\quad\text{for $1\le j\le n$.}
\]
in the diagonal case.  Rearranging these equations leads to the formulae
\[
r_{ij}=\frac{1}{r_{jj}}\biggl(a_{ij}-\sum_{k=\max(1,j-s)}^{i-1}r_{ki}r_{kj}
	\biggr)\quad\text{for $\max(1,j-s)\le i\le j\le n$,}
\]
and
\[
r_{jj}=\sqrt{a_{jj}-\sum_{k=\max(1,j-s)}^{j-1}r_{kj}^2}
	\quad\text{for $1\le j\le n$,}
\]
...
(\boldsymbol{R}^T\boldsymbol{y})_i=\sum_{j=1}^nr_{ji}y_j
	=\sum_{j=\max(1,i-s)}^ir_{ji}y_j
	=r_{ii}y_i+\sum_{j=\max(1,i-s)}^{i-1}r_{ji}y_j
\]
and so $\boldsymbol{R}^T\boldsymbol{y}=\boldsymbol{b}$ iff
\[
y_i=\frac{1}{r_{ii}}\biggl(b_i-\sum_{j=\max(1,i-s)}^{i-1}r_{ji}y_j\biggr)
\quad\text{for $1\le i\le n$.}
\]
Similarly, since $r_{ij}=0$ if $i>j$~or $j-i>s$,
\[
(\boldsymbol{R}\boldsymbol{x})_i=\sum_{j=1}^nr_{ij}x_j
	=\sum_{j=i}^{\min(n,i+s)}r_{ij}x_j
	=r_{ii}x_i+\sum_{j=i+1}^{\min(n,i+s)}r_{ij}x_j
\]
and so $\boldsymbol{R}\boldsymbol{x}=\boldsymbol{y}$ iff
\[
x_i=\frac{1}{r_{ii}}\biggl(y_i-\sum_{j=i+1}^{\min(n,i+s)}r_{ij}x_j\biggr)
	\quad\text{for $1\le i\le n$.}
\]
These formula lead to Algorithm~\ref{alg: band Chol solve} for solving 
$\boldsymbol{A}\boldsymbol{x}=\boldsymbol{b}$.

When $s$ is small compared to~$n$, storing $\boldsymbol{A}$ as an 
$n\times n$~array wastes a lot of space.  A more efficient method is to use 
the \emph{symmetric band storage scheme} in which an 
$(s+1)\times n$~matrix~$\boldsymbol{A}_{\textrm{b}}$ holds the non-zero 
superdiagonals of~$\boldsymbol{A}$ by putting
\[
(\boldsymbol{A}_{\mathrm{b}})_{s+1+i-j,j}=a_{ij}
	\quad\text{for $\max(1,j-s)\le i\le j$ and $1\le j\le n$.}
\]
For example, if $n=9$ and $s=3$, then
\[
