...
of~$\mathbb{R}^2$, with a piecewise smooth boundary~$\partial\Omega$.  Given 
suiatble functions $f(x,y)$~and $g(x,y)$, we see $u=u(x,y)$ satisfying
\begin{equation}\label{eq: Poisson bvp}
\begin{aligned}
-\nabla^2u&=f(x,y)&&\text{for $(x,y)\in\Omega$,}\\
u&=g(x,y)&&\text{for $(x,y)\in\partial\Omega$.}
\end{aligned}
\end{equation}
...
U_{p,q}&=g_{p,q}&&\text{for $(x_p,y_q)\in\partial\Omega$,}
\end{aligned}
\end{equation}
