...
density of heat sources, with $\gamma_0$~and $\gamma_L$ the ambient
temperatures at $x=0$~and $x=L$, respectively.

For a constant source term~$f(x)=c$, the solution is easily seen to be
...
\end{equation}
Notice $(\boldsymbol{L}\boldsymbol{D}\boldsymbol{L}^\top)^\top
=(\boldsymbol{L}^\top)^\top\boldsymbol{D}^\top\boldsymbol{L}^\top
=\boldsymbol{L}\boldsymbol{D}\boldsymbol{L}^\top$, so the
factorisation~\eqref{eq: L D LT} is not possible if
$\boldsymbol{A}^\top\ne\boldsymbol{A}$.
...
\]
Since $\boldsymbol{L}$ is \emph{unit lower triangular} and $\boldsymbol{D}$ is
...
By exploiting the maximum principle, we are able to derive the following upper
and lower bounds on a solution of the ODE.
...
\]
and observe that $\nu'(x)=-\mu\sinh\mu(x-L/2)$ is positive for~$x<L/2$ and
negative for~$x>L/2$.  Since $\nu(0)=0=\nu(L)$, it follows that
$v\ge0$ on~$[0,L]$ with
\begin{align*}
...
\begin{theorem}\label{thm: perturbed bvp 1d}
...
$(\mathcal{L}_{\Delta x}W)_p<0$ for~$1\le p\le P-1$.  By
...
discrete boundary-value problem~\eqref{eq: finite diff Dirichlet 1d} has a
unique solution~$U_p$, and
\[
...
then
\[
\mathcal{E}(\Delta x)\approx\Bigl(\max_{0\le x\le L}|v(x)|\Bigr)\,\Delta x^r.
\]
...
Let $e_P=\mathcal{E}(\Delta x)=\max_{0\le p\le P}|U_p-u(x_p)|$ denote the
maximum error in the finite difference solution for a grid with
$P$~subintervals.  The argument above shows that if the error behaves as
in~\eqref{eq: Ep asymp} then
\begin{equation}\label{eq: eP asymp}
...
\]
\item{(iii)}
Find $v_1$~and $v_2$, and then use the boundary conditions $u(0)=\gamma_0$~and
...
\exercise
If $\boldsymbol{A}$ is a symmetric tridiagonal matrix, then instead of the 
