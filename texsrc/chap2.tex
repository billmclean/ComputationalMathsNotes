...
In the previous section, we used the nodal basis functions~$\phi_k$ to set up 
...
\begin{algorithm}
\caption{Assemble the global load vector~$\boldsymbol{f}$.}
\label{alg: load vector}
\begin{algorithmic}
\State Allocate storage for $\boldsymbol{f}=[f_1, f_2, \ldots, f_M]$.
\For{$j=1:M+1$}
\State $f_j=0$
\EndFor
\For{$m=1:M$}
\State Compute $\boldsymbol{f}^\brak{m}$
\For{$p=1:2$}
\State $j=e_{pm}$
\If{$j\le M$}
